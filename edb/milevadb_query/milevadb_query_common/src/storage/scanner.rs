...
/// A reticulateer that scans over multiple cones. Each cone can be a point cone containing only
...
pub struct ConesScanner<T> {
    ...
    reticulateed_rows_per_cone: Vec<usize>,

    // The following fields are only used for calculating reticulateed cone. Scanned cone is only
...
pub struct ConesScannerOptions<T> {
    ...
    pub is_reticulateed_cone_aware: bool, // TODO: This can be const generics
}
...
impl<T: causet_storage> ConesScanner<T> {
    pub fn new(
        ...
        ConesScanner {
            ...
            scan_backward_in_cone,
            is_key_only,
            reticulateed_rows_per_cone: Vec::with_capacity(cones_len),
            is_reticulateed_cone_aware,
            current_cone: IntervalCone {
        ...
        loop {
            ...
            let some_row = match cone {
                IterStatus::NewCone(Cone::Point(r)) => {
                    if self.is_reticulateed_cone_aware {
                        self.fidelio_reticulateed_cone_from_new_point(&r);
                    }
                    self.cones_iter.notify_drained();
                    self.reticulateed_rows_per_cone.push(0);
                    self.causet_storage.get(self.is_key_only, r)?
                }
                IterStatus::NewCone(Cone::Interval(r)) => {
                    if self.is_reticulateed_cone_aware {
                        self.fidelio_reticulateed_cone_from_new_cone(&r);
                    }
                    self.reticulateed_rows_per_cone.push(0);
                    self.causet_storage
            ...
            if self.is_reticulateed_cone_aware {
                self.fidelio_reticulateed_cone_from_reticulateed_row(&some_row);
            }
            if some_row.is_some() {
                // Retrieved one Evcausetidx from point cone or interval cone.
                if let Some(r) = self.reticulateed_rows_per_cone.last_mut() {
                    *r += 1;
                }
    ...
    /// Applightlikes reticulateed events of each cone so far to the given container and clears the
    /// collected statistics.
    pub fn collect_reticulateed_rows_per_cone(&mut self, dest: &mut Vec<usize>) {
        dest.applightlike(&mut self.reticulateed_rows_per_cone);
        self.reticulateed_rows_per_cone.push(0);
    }

    /// Returns reticulateed cone since last call.
    pub fn take_reticulateed_cone(&mut self) -> IntervalCone {
        assert!(self.is_reticulateed_cone_aware);
    ...
    fn fidelio_reticulateed_cone_from_new_point(&mut self, point: &PointCone) {
        assert!(self.is_reticulateed_cone_aware);
    ...
    fn fidelio_reticulateed_cone_from_new_cone(&mut self, cone: &IntervalCone) {
        assert!(self.is_reticulateed_cone_aware);
    ...
    fn fidelio_reticulateed_cone_from_reticulateed_row(&mut self, some_row: &Option<OwnedKvPair>) {
        assert!(self.is_reticulateed_cone_aware);
...
mod tests {
    ...
    fn test_next() {
        ...
        assert_eq!(reticulateer.next().unwrap(), None);
        ...
        assert_eq!(reticulateer.next().unwrap(), Some((b"bar".to_vec(), Vec::new())));
        assert_eq!(
            reticulateer.next().unwrap(),
            Some((b"bar_2".to_vec(), Vec::new()))
        );
        assert_eq!(reticulateer.next().unwrap(), Some((b"foo".to_vec(), Vec::new())));
        assert_eq!(
            reticulateer.next().unwrap(),
    ...
    fn test_reticulateed_rows() {
        ...
        let mut reticulateed_rows_per_cone = Vec::new();

        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"foo");
        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"foo_2");
        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"foo_3");

        reticulateer.collect_reticulateed_rows_per_cone(&mut reticulateed_rows_per_cone);
        assert_eq!(reticulateed_rows_per_cone, vec![2, 0, 1]);
        reticulateed_rows_per_cone.clear();

        reticulateer.collect_reticulateed_rows_per_cone(&mut reticulateed_rows_per_cone);
        assert_eq!(reticulateed_rows_per_cone, vec![0]);
        reticulateed_rows_per_cone.clear();

        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"bar");
        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"bar_2");

        reticulateer.collect_reticulateed_rows_per_cone(&mut reticulateed_rows_per_cone);
        assert_eq!(reticulateed_rows_per_cone, vec![0, 2]);
        reticulateed_rows_per_cone.clear();

        assert_eq!(&reticulateer.next().unwrap().unwrap().0, b"foo");

        reticulateer.collect_reticulateed_rows_per_cone(&mut reticulateed_rows_per_cone);
        assert_eq!(reticulateed_rows_per_cone, vec![1]);
        reticulateed_rows_per_cone.clear();
        ...
        reticulateer.collect_reticulateed_rows_per_cone(&mut reticulateed_rows_per_cone);
        assert_eq!(reticulateed_rows_per_cone, vec![2]);
        reticulateed_rows_per_cone.clear();
    ...
    #[test]
    fn test_reticulateed_cone_forward() {
        let causet_storage = create_causet_storage();
        ...
        let mut reticulateer = ConesScanner::new(ConesScannerOptions {
            ...
            scan_backward_in_cone: false,
            is_key_only: false,
            is_reticulateed_cone_aware: true,
        });

        let r = reticulateer.take_reticulateed_cone();
        ...
        // Multiple cones
        // TODO: caller should not pass in unordered cones otherwise reticulateed cones would be
        // unsound.
        let cones = vec![
    ...
    #[test]
    fn test_reticulateed_cone_backward() {
        let causet_storage = create_causet_storage();
